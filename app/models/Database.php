...
class Database
{
  ...
  private function __construct()
  {
    if ($this->connection === null) {
      $this->connection = new PDO("pgsql:dbname=" . Config::get("DB_NAME") . ";host=" . Config::get("DB_HOST") . ";port=" . Config::get("DB_PORT"), Config::get("DB_USER"), Config::get("DB_PASS"));
  ...
  {
    $this->statement = $this->connection->prepare("SELECT COUNT(*) AS count FROM data." . $table . " WHERE " . $columnName . " = " . $noAcct);
    $this->execute();
    return (int) $this->fetchAssociative()["count"];
  }

  public function countReviewByNoAcct($noAcct)
  {
    $this->statement = $this->connection->prepare("SELECT COUNT(*) AS count FROM data.pindaan_raw WHERE no_akaun = '" . $noAcct . "'");
    $this->execute();
    return (int) $this->fetchAssociative()["count"];
  }
  ...
  /**
   * Select a row from a table provided by no account
   *
   * @access public
   * @param   string $table
   * @param   mixed  $id
  ...
  public function getReviewByNoAcct($noAcct)
  {
    if ($this->countReviewByNoAcct($noAcct) > 0) {
      $this->statement = $this->connection->prepare("SELECT * FROM data.pindaan_raw WHERE no_akaun = '" . $noAcct . "' LIMIT 1");
      $this->execute();
    }
  }
  ...
  /**
   * Select all rows from a table provided by worker id
   *
   * @access public
   * @param   string  $table
   ...
   *
   */
  public function getByUserWorkerId($table, $worker_id)
  {
    $this->statement = $this->connection->prepare("SELECT * FROM " . $table . " WHERE worker_id = :worker_id");
    $this->bindValue(":worker_id", $worker_id);
    $this->execute();
  }
  ...
  public function getDataByTableColumnAndWhere($table, $column, $where)
  {
    $this->statement = $this->connection->prepare("SELECT " . $column . " FROM " . $table . " WHERE " . $where . " GROUP BY " . $column);
    $this->execute();
  }
  ...
  public function deleteByIdCustom($table, $field, $fileId)
  {
    $this->statement = $this->connection->prepare("DELETE FROM " . $table . " WHERE " . $field . " = :id");
    $this->bindValue(":id", $fileId);
    $this->execute();
  }

  public function deleteByUserId($table, $id)
  {
    $this->statement = $this->connection->prepare("DELETE FROM " . $table . " WHERE user_id = :id");
    $this->bindValue(":id", $id);
    $this->execute();
  }

  /**
   * Update all user activity
   *
   */

  public function logActivity($userId, $activity)
  {
    $this->statement = $this->connection->prepare("INSERT INTO public.activity(userid, activity) VALUES (:userid, :activity)");
    $this->bindValue(":userid", $userId);
    $this->bindValue(":activity", $activity);
    $this->execute();
  }
  ...
  public function updateStatusSubmitted($id)
  {
    $this->statement = $this->connection->prepare('UPDATE data.smktpk SET smk_stspn = 2 WHERE id = :id');
    $this->bindValue(":id", $id);
    $this->execute();
  }
