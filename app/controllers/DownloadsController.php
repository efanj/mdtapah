...
class DownloadsController extends Controller
{
  ...
  public function download($hashedFileName = "")
  {
    ...
    $file["basename"] = str_replace(" ", "_", $file["filename"]) . "." . $file["extension"];

    if (!Uploader::isFileExists($fullPath)) {
      return $this->error(404);
    }

    $this->response->download($fullPath, ["basename" => str_replace(" ", "_", $file["filename"]), "extension" => $file["extension"]]);
  }

  public function downloadBenchMark($hashedFileName = "")
  {
    $fullPath = IMAGES . "documents/";
    $file = $this->file->getByBenchHashedName($hashedFileName);
