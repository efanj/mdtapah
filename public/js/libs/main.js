...
var events = {
  ...
  account: {
    init: function () {
      events.account.eliminated()
    },
    eliminated: function () {
      $("#form-jadualA").submit(function (e) {
        e.preventDefault()
        ajax.send("Account/createA", helpers.serialize(this), getEliminatedCallBack)
      })

      function getEliminatedCallBack(result) {
        if (result.success === true) {
          swal(
            {
              title: "Berjaya!",
              text: "Jadual A, Telah berjaya direkodkan.",
              icon: "success",
              button: true
            },
            function () {
          ...
          swal("Oops...", "Jadual A, tidak berjaya direkodkan!", "error")
        }
      }
    }
  },
  ...
  amendment: {
    init: function () {
      events.amendment.review()
    },
    review: function () {
      $("#form-review").submit(function (e) {
        e.preventDefault()
        ajax.send("Amendment/getReviewTable", helpers.serialize(this), getReviewTableCallBack)
      })

      function getReviewTableCallBack(result) {
        if (helpers.validateData(result, "#reviewlists", "html", "default", "data")) {
          console.log(result.data.review)
          $("#reviewlists tbody").html(result.data.review)
        }
      }
    }
  },
  ...
  vendor: {
    ...
    desktopreview: function () {
      $("#form-semak-desktop").submit(function (e) {
        e.preventDefault()
        ajax.send("Vendor/createReviewDesktop", helpers.serialize(this), createReviewCallBack)
      })

      function createReviewCallBack(result) {
        if (result === true) {
          swal(
            {
              title: "Berjaya!",
              text: "Semakan desktop, Telah berjaya direkodkan.",
              icon: "success",
    ...
    uploadbenchmarkdocs: function () {
      $("#form-upload-docs-benchmark").submit(function (e) {
        e.preventDefault()

        if (helpers.validateFileSize("file")) {
          ajax.upload("Vendor/uploadbenchmarkdocs", new FormData(this), createBenchMarkDocsCallBack)
        } else {
          helpers.displayError("#form-upload-docs-benchmark", "File size can't exceed max limit")
        }
      })

      function createBenchMarkDocsCallBack(result) {
        if (helpers.validateData(result, "#form-upload-docs-benchmark", "after", "default", "data")) {
          $(".gallery .no-data").remove()
          $(result.data).hide().prependTo(".gallery").fadeIn()
          events.vendor.reInit()
        }
      }
    },
    uploadimages: function () {
      $("#form-upload-file").submit(function (e) {
        ...
        if (helpers.validateFileSize("file")) {
          ajax.upload("Vendor/uploadimages", new FormData(this), createFileCallBack)
        } else {
    ...
    uploaddocuments: function () {
      $("#form-upload-docs").submit(function (e) {
        e.preventDefault()

        if (helpers.validateFileSize("file")) {
          ajax.upload("Vendor/uploaddocuments", new FormData(this), createDocsCallBack)
        } else {
          helpers.displayError("#form-upload-docs", "File size can't exceed max limit")
        }
      })

      function createDocsCallBack(result) {
        if (helpers.validateData(result, "#form-upload-docs", "after", "default", "data")) {
          $(".gallery .no-data").remove()
    ...
    deletebenchdocuments: function () {
      $(".delete-benchdoc").on("click", function (e) {
        var panel = $(this).closest(".docPanel")
        var docId = $(this).data("id")
        var postData = { doc_id: docId }
        bootbox.confirm({
          message: "Dokumen dipadamkan.",
          ...
          callback: function (res) {
            if (res) {
              $.ajax({
                type: "POST",
                url: config.root + "Vendor/deletebenchdocument",
                data: helpers.appendCsrfToken(postData),
    ...
    deleteimages: function () {
      $(".delete-image").on("click", function (e) {
        var panel = $(this).closest(".imagePanel")
        var imageId = $(this).data("id")
        var postData = { image_id: imageId }
        bootbox.confirm({
          message: "Gambar dipadamkan.",
          title: "Anda pasti, ingin memadamkannya ?",
          className: "modal-style2",
          callback: function (res) {
            if (res) {
              $.ajax({
                type: "POST",
                url: config.root + "Vendor/deleteimage",
                data: helpers.appendCsrfToken(postData),
                success: function (data) {
                  panel.remove()
                }
        ...
        bootbox.confirm({
          ...
          callback: function (res) {
            if (res) {
              $.ajax({
                type: "POST",
                url: config.root + "Vendor/deletedocument",
                data: helpers.appendCsrfToken(postData),
                success: function (data) {
                  panel.remove()
                }
  ...
  users: {
    ...
    get: function (pageNumber) {
      ...
      ajax.send("Admin/getUsers", { name: name, email: email, role: role, page: pageNumber }, events.users.get_search_callback, "#list-users")
    },
    search: function () {
    ...
    clear: function () {
      $("#clearlog").click(function (e) {
        ...
        ajax.send("Admin/clearLog", {}, deleteUserCallBack)
        function deleteUserCallBack(result) {
          if (result.success === true) {
            swal("Berjaya", "Log ralat berjaya dipadamkan", "success")
          } else {
            swal("Oops...", "Log ralat tidak berjaya dipadamkan", "error")
          }
        }
      })
    }
