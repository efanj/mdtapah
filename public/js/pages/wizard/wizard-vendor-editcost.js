$(document).ready(function () {
  // var $validator = $("#editCalcBuilding form").validate({
  ...
  //init first wizard
  $("#edit-calc-cost").bootstrapWizard({
    ...
    onTabShow: function (tab, navigation, index) {
      ...
      if ($current >= $total) {
        $("#edit-calc-cost").find(".pager .next").hide()
        $("#edit-calc-cost").find(".pager .finish").show()
        $("#edit-calc-cost").find(".pager .finish").removeClass("disabled")
      } else {
        $("#edit-calc-cost").find(".pager .next").show()
        $("#edit-calc-cost").find(".pager .finish").hide()
      }
    }
  })

  //wizard is finish
  $("#edit-calc-cost .finish").click(function (e) {
    e.preventDefault()
    var data = $("#editCalcCost").serialize()
    $.ajax({
      url: config.root + "calculator/costEdit",
      type: "post",
      dataType: "json",
      data: data
      ...
      if (result === true) {
        swal(
          {
            title: "Berjaya!",
            text: "Nilaian Bangunan, Telah berjaya dikemaskini.",
            icon: "success",
  ...
  $("#edit-calc-cost #update_rate").click(function (e) {
    e.preventDefault()
    var rate = $("#rate").val()
    var kwkod = $("#kwkod").val()
    var htkod = $("#htkod").val()
